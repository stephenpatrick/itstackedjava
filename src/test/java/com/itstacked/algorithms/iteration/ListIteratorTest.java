...
public class ListIteratorTest extends BaseListIteratorTest {
	...
	public void first() {
		...
		Assert.assertEquals(iterator.current(), sampleList.get(0));
	}
	...
	public void next() {
		...
		Assert.assertFalse("Expected not to be done", iterator.isDone());
		assertIteration(iterator, true, true, sampleList);
		Assert.assertTrue("Expected to be done", iterator.isDone());
	}
	...
	public void previous() {
		...
		iterator.last();
		Assert.assertFalse("Expected not to be done", iterator.isDone());
		assertIteration(iterator, false, false, sampleList);
		Assert.assertTrue("Expected to be done", iterator.isDone());
	}
	...
	public void previous_IndexOutBounds() {
		...
		Assert.assertEquals(iterator.current(), Integer.valueOf(1));
		iterator.previous();
	...
	private Iterator<Integer> createTestIterator() {
		return new ListIterator<Integer>(sampleList);
	}
}
